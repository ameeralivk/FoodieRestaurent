...
import { getItem } from "../../services/user"; // Removed sendToAi
...
const ItemDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRating = async () => {
      if (!item) return;
      try {
        const res = await getRating(restaurantId ? restaurantId : "");
        if (res.success) {
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] p-4 md:px-8 z-50 rounded-t-3xl md:rounded-none">
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-between px-6 py-4 rounded-2xl font-bold text-white shadow-lg transition-all hover:scale-[1.02] active:scale-95 ${
              item.isStock
