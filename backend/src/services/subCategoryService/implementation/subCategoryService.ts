...
export class SubCategoryService implements ISubCategoryService {
  ...
  async addSubCategory(data: IZodSubCategory,restaurantId:string,categoryId:string): Promise<ISubCategory> {
    ...
    return this._subCategoryRepo.createSubCategory(data,restaurantObjId,categoryObjId);
  }



   async editSubCategory(id: string, data:IZodSubCategory):Promise<ISubCategory> {
   ...
   async deleteSubCategory(categoryId: string):Promise<ISubCategory> {
    const deleted = await this._subCategoryRepo.softDeleteSubCategory(categoryId);
    if (!deleted) {
   ...
   async getAllSubCategories(
    ...
    const allSubcategory = await this._subCategoryRepo.getAllSubCategories(filter, page, limit);
    return {success:true,data:allSubcategory.data,total:allSubcategory.total}
  }
  ...
  page: number,
  limit: number
):Promise<{data:ISubCategoryDTO[],total:number,page:number,limit:number}> {
 const restaurantObjId = new Types.ObjectId(restaurantId)
