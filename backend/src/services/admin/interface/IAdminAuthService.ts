...
export default interface IAdminAuthService {
  ...
  verifyOtp(
    email: string,
    otp: string
  ): Promise<{ success: boolean; message: string ,data:{admin:AdminDocument},accesstoken:string}>;
  resendOtp(
    email:string
  ):Promise<{success:boolean,message:string}>
  googleAuth(accessToken:string):Promise<{admin:IAdmin,accesstoken:string,refreshToken:string}>;
